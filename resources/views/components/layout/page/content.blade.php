@props(['header'=>''])
    ...
    dark:scrollbar-thumb-gray-500 dark:scrollbar-track-gray-700">
        <!-- Main content header -->
        <div class="flex flex-col justify-center xs:items-center sm:items-start justify-between py-4 space-y-4 border-b dark:border-gray-700 lg:items-center lg:space-y-0 lg:flex-row">
            <h1 class="flex text-2xl font-semibold whitespace-nowrap text-blue-500 dark:text-yellow-500 h-full px-5">{{$header}}</h1>
            @if(!request()->is('dashboard') )
            <a href="{{ url()->previous() }}" type="button" class="py-2.5 px-5 mr-5 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-full
