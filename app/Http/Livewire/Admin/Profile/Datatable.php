...
//                $filename = $this->user['photo']->storeAs('public/images/profile', $newFileName);
    ...
    {
        if (Data::is_user_guard_admin()) return (Admin::select('id', 'name', 'email')->where('id', auth()->user()->id)->first());
        if (Data::is_user_guard_web()) return (User::select('id', 'name', 'email')->where('id', auth()->user()->id)->first());
    }
    ...
    public function render()
    {
        ...
        return view('livewire.admin.profile.datatable', ['profile' => $profile, 'gender' => $gender,
